...
from ..utils import load_env_variable, load_csv_to_sqlite
...
# Prepare SQLite database in memory
conn = sqlite3.connect(":memory:")  # In-memory SQLite database

# Dictionary to store loaded DataFrames for each CSV file (allowing multiple datasets)
...
def get_table_schema(conn: sqlite3.Connection, table_name: str):
    ...
    """
    # Ensure the table name is quoted properly to prevent SQL injection or errors
